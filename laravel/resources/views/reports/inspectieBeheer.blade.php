        ...
        <nav class="flex flex-wrap gap-2">
            @foreach ($tabs as $key => $tab)
                <a href="{{ request()->fullUrlWithQuery(['status' => $key, 'page' => null]) }}"
                    class="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm {{ $active === $key ? 'bg-sky-600 text-white' : 'bg-white border text-gray-700 hover:bg-gray-50' }}">
                    <span>{{ $tab['label'] }}</span>
                    <span class="inline-flex items-center justify-center min-w-6 h-6 rounded-full text-xs {{ $active === $key ? 'bg-white/20' : 'bg-gray-100 text-gray-700' }}">
                        {{ $tab['count'] }}
                    </span>
                </a>
            @endforeach
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Controles</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Aangemaakt</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Bijgewerkt</th>
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-sm text-gray-500">#{{ $list->id }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-gray-700">
                                    {{ optional($list->updated_at)->diffForHumans() ?? '—' }}
                                </td>
                                <td class="px-6 py-4 text-right space-y-2 sm:space-y-0 sm:space-x-2">
